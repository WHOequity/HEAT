...
table_compare_summary <- function(.data, columns, decimal_places, focus_setting,
  ...
  if (!data_only) {
    ...
    columns_selection <- c(
      ...
      "dimension", "measure_abbr", "measure_name", "estimate", "se",
      "ci_lb", "ci_ub", "setting_average", "iso3"
    )
  }

  if (data_only) {
    columns_selection <- c(columns_selection, "iso3")
  }
